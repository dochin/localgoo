...
goobackup() {
	# Create an initial backup of the existing site 
...
goosearch() {
	...
	#Remove duplicates
	sort -u download.tmp > download2.tmp
	mv download2.tmp download.tmp
	
	#save the download an files lists, remove temp files
	cat download.tmp >> "$CONFIG_DIR/download.list"
	cat unique.tmp >> "$CONFIG_DIR/files.list"
	cleanup
}
...
goonukem() {
	...
	if [ -f "$CONFIG_DIR/download.list" ]; then
		echo "You have downloads queued.  Run './localgoo.sh download' first."
	elif [ -f "$CONFIG_DIR/files.list" ]; then
		#first do a backup
		goobackup
